...
//smaller delay for testing purposes
const long LIGHT_DELAY_MS = 1000L; // time for which the light shall stay on when a motion is detected (millis)
const long FADE_DELAY_MS = 3L; // _delay_ms at each step of fading

const byte ON_VALUE = 255; //this should be the same as the max possible value to use the full brightness
...
void setup() {
    ...
    //might have changed it
     ...
     ICR1 |= 0xFFFF; // set timers to use 8bit (count to 0x00FF) because of current pwm table
...
void fade(int target_brightness_level) {
    if(target_brightness_level == current_brightness) {
        ...
        while(current_brightness > target_brightness_level) {
            OCR1A = pwmtable_16[--current_brightness]; // reduce current_brightness and apply to register
            Serial.println(pwmtable_16[current_brightness]);
            delay(FADE_DELAY_MS);
        }
    } else if(target_brightness_level > current_brightness) { // fade up
        while(current_brightness < target_brightness_level) {
            OCR1A = pwmtable_16[++current_brightness]; // increase current_brightness and apply to register
            Serial.println(pwmtable_16[current_brightness]);
            delay(FADE_DELAY_MS);
        }
    }
